...
class G318Map {
    ...
    zoomToSegment(segment) {
        ...
        switch (segment) {
            case 'east':
                segmentPoints = routePoints.slice(0, 6); // 上海到武汉
                break;
            case 'middle':
                segmentPoints = routePoints.slice(5, 16); // 武汉到成都
                break;
            case 'west':
                segmentPoints = routePoints.slice(15); // 成都到拉萨
                break;
            default:
                this.map.fitWorld();
                return;
        ...
        if (segmentPoints && segmentPoints.length > 0) {
            ...
            this.map.flyToBounds(bounds, { padding: [50, 50], duration: 2.5 });
        }
    }
